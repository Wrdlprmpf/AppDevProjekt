...
class SpeedometerActivity : AppCompatActivity() , LocationListener, SensorEventListener {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		userName = CompanionUserData.user.name
		...
		units1.text = CompanionUserData.unit
		units2.text = CompanionUserData.unit
		units3.text = CompanionUserData.unit
		...
		startBtn.setOnClickListener {
			...
			Toast.makeText(applicationContext,"Recording startet", Toast.LENGTH_SHORT).show()
	...
	private fun companionData(){
		CompanionUserData.user.average_speed = averageSpeed
		CompanionUserData.user.top_speed = topSpeed
	}

	private fun insertDataToDatabase() {

		if(topSpeed > CompanionUserData.user.top_speed){
			// Create User Object
			CompanionUserData.user.top_speed =  topSpeedOutput.text.toString().toFloat()
			CompanionUserData.user.average_speed = averageSpeedOutput.text.toString().toFloat()
			// Add Data to Database
			myUserViewModel.updateUser(CompanionUserData.user)
			Toast.makeText(applicationContext, "Congratulation you got faster", Toast.LENGTH_LONG).show()
			// Navigate Back
		}else{
			Toast.makeText(applicationContext, "No new record", Toast.LENGTH_LONG).show()
		}
	}
